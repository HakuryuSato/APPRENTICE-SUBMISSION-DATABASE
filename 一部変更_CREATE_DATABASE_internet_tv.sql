-- インターネットTV用データベースを構築するコマンド列

-- トランザクション開始 *問題があればROLLBACKで中断すること
START TRANSACTION;
...
CREATE INDEX `video_id_idx` ON `internet_tv`.`program_genres` (`program_id` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `internet_tv`.`episodes` (
  ...
  `episode_view_counts` INT NOT NULL,
